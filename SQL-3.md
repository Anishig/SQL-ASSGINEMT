...
3.
SELECT P.PARTY_ID,P.FIRST_NAME FROM RETURN_ITEM AS RI
JOIN RETURN_HEADER AS RH ON RH.RETURN_ID = RI.RETURN_ID
JOIN PERSON AS P ON P.PARTY_ID = RH.FROM_PARTY_ID
WHERE RH.ENTRY_DATE >= DATE_SUB(DATE_SUB(CURRENT_DATE(), INTERVAL DAYOFMONTH(CURRENT_DATE())-1 DAY), INTERVAL 1 MONTH) AND RH.ENTRY_DATE < DATE_SUB(CURRENT_DATE(), INTERVAL DAYOFMONTH(CURRENT_DATE())-1 DAY)
GROUP BY RI.ORDER_ID, RH.FROM_PARTY_ID HAVING COUNT(RI.ORDER_ID) = 1;
...
12.
SELECT OH.ORDER_ID ,OH.ORDER_DATE ,OH.STATUS_ID ,P.FACILITY_ID ,DATEDIFF(P.PICKLIST_DATE, OH.ORDER_DATE) AS DURATION FROM ORDER_HEADER AS  OH
JOIN PICKLIST_BIN AS PB ON OH.ORDER_ID = PB.PICKLIST_BIN_ID
LEFT JOIN  PICKLIST AS  P ON P.PICKLIST_ID = PB.PICKLIST_ID
```
